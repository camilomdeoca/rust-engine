...
impl LightCullingPass {
    pub fn new(context: &RendererContext, settings: &Settings) -> Result<Self, RendererError> {
        ...
        let light_culling_pipeline = create_light_culling_pipeline(
            ...
            settings.renderer.light_culling.max_lights_per_tile,
        )?;
    ...
    pub fn execute(
        ...
        let next_ligth_index_global_storage_buffer = context
        ...
        let num_tiles = UVec3::new(
            resolution
                .x
                .div_ceil(settings.renderer.light_culling.tile_size),
            resolution
                .y
                .div_ceil(settings.renderer.light_culling.tile_size),
            settings.renderer.light_culling.z_slices,
        );

        let visible_light_indices_storage_buffer = context
            .in_device_storage_buffer_allocator
            .allocate_slice::<u32>(
                (num_tiles.x
                    * num_tiles.y
